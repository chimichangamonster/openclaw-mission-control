...
const MODEL_PRICING: Record<string, { prompt: number; completion: number }> = {
  ...
  "deepseek-chat-v3": { prompt: 0.4, completion: 1.3 },
  "deepseek-chat-v3.1": { prompt: 0.2, completion: 0.8 },
  "gemini-2.5-flash": { prompt: 0.3, completion: 2.5 },
  "gemini-2.5-flash-lite": { prompt: 0.1, completion: 0.4 },
  "gpt-5-nano": { prompt: 0.05, completion: 0.4 },
  "gpt-4o-mini": { prompt: 0.15, completion: 0.6 },
  "minimax-m2.7": { prompt: 0.3, completion: 1.2 },
  "kimi-k2.5": { prompt: 0.45, completion: 2.2 },
};
...
function estimateCost(model: string, inputTokens: number, outputTokens: number): number {
  ...
  if (!pricing) return 0;
  return (inputTokens / 1_000_000) * pricing.prompt + (outputTokens / 1_000_000) * pricing.completion;
}
    ...
    >
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
            <div className="border-b border-slate-100 px-4 py-3">
              <h3 className="text-sm font-semibold text-slate-900">Model Pricing Reference (per 1M tokens)</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                ...
                <tbody>
                  {Object.entries(MODEL_PRICING)
                    .sort((a, b) => a[1].prompt - b[1].prompt)
                    .map(([model, price]) => (
                      <tr key={model} className="border-b border-slate-50 hover:bg-slate-50">
                        <td className="px-4 py-2 font-mono text-xs text-slate-700">{model}</td>
                        <td className="px-4 py-2 text-right font-mono text-xs text-slate-600">${price.prompt.toFixed(2)}</td>
                        <td className="px-4 py-2 text-right font-mono text-xs text-slate-600">${price.completion.toFixed(2)}</td>
                        <td className="px-4 py-2">
                          <span className={`rounded px-1.5 py-0.5 text-xs font-medium ${
                            price.prompt >= 3 ? "bg-purple-100 text-purple-700" :
                            price.prompt >= 0.2 ? "bg-blue-100 text-blue-700" :
                            "bg-emerald-100 text-emerald-700"
                          }`}>
                            {price.prompt >= 3 ? "Premium" : price.prompt >= 0.2 ? "Mid" : "Budget"}
                          </span>
                        </td>
                      </tr>
                    ))}
