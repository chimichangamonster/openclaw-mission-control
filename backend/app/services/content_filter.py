...
_CN_FILTER_RULES: list[tuple[re.Pattern[str], str]] = [
    ...
    (
        re.compile(
            r"overthrow\s+(?:the\s+)?(?:Chinese|PRC)\s+(?:government|state|regime)", re.IGNORECASE
        ),
        "state_security",
    ),
    ...
    (
        re.compile(r"Tiananmen\s+(?:Square\s+)?(?:massacre|protest|crackdown)", re.IGNORECASE),
        "sensitive_history",
    ),
    (
        re.compile(r"June\s+(?:4th|Fourth)\s+(?:incident|massacre|crackdown)", re.IGNORECASE),
        "sensitive_history",
    ),
    # Political leadership criticism (direct insults/attacks)
    (re.compile(r"习近平.*(?:独裁|暴君|下台)", re.IGNORECASE), "political_leadership"),
    (
        re.compile(r"Xi\s+Jinping\s+.*(?:dictator|tyrant|step\s+down|resign)", re.IGNORECASE),
        "political_leadership",
    ),
    # Ethnic / religious incitement
    (re.compile(r"(?:维吾尔|藏|回|蒙)族.*(?:劣等|消灭|清洗)", re.IGNORECASE), "ethnic_incitement"),
    (
        re.compile(r"(?:Uyghur|Tibetan|Hui)\s+.*(?:inferior|eliminate|cleanse)", re.IGNORECASE),
        "ethnic_incitement",
    ),
    ...
    (
        re.compile(
            r"(?:how\s+to\s+)?(?:make|build|assemble)\s+(?:a\s+)?(?:bomb|explosive|weapon)",
            re.IGNORECASE,
        ),
        "violence",
    ),
    ...
    (
        re.compile(r"(?:bypass|circumvent)\s+(?:the\s+)?(?:Great\s+)?Firewall", re.IGNORECASE),
        "circumvention",
    ),
]
