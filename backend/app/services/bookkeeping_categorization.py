...
_VENDOR_RULES: list[tuple[re.Pattern, str]] = [
    (re.compile(r"home\s*depot|lowes|rona|totem|building\s*supplies|lumber", re.IGNORECASE), "materials"),
    (re.compile(r"shell|esso|petro|husky|co-op.*gas|pioneer|7-?eleven.*gas|fas\s*gas", re.IGNORECASE), "fuel"),
    ...
    (re.compile(r"tim\s*horton|subway|mcdonald|a&w|wendy|pizza|restaurant|coffee", re.IGNORECASE), "food"),
...
_ITEM_RULES: list[tuple[re.Pattern, str]] = [
    (re.compile(r"lumber|plywood|drywall|concrete|cement|rebar|insulation|shingle|roofing|siding|nail|screw|bolt", re.IGNORECASE), "materials"),
    ...
    (re.compile(r"glove|hard\s*hat|helmet|vest|boot|gogg|ear.*plug|respirator", re.IGNORECASE), "ppe"),
]
