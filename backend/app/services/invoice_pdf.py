...
def generate_invoice_pdf(
    ...
    # Styles
    title_style = ParagraphStyle("InvTitle", parent=styles["Heading1"], fontSize=28, textColor=colors.HexColor("#1a1a2e"), spaceAfter=4)
    heading_style = ParagraphStyle("InvHeading", parent=styles["Heading2"], fontSize=12, textColor=colors.HexColor("#555555"), spaceAfter=2)
    normal_style = ParagraphStyle("InvNormal", parent=styles["Normal"], fontSize=10, leading=14)
    bold_style = ParagraphStyle("InvBold", parent=styles["Normal"], fontSize=10, leading=14, fontName="Helvetica-Bold")
    small_style = ParagraphStyle("InvSmall", parent=styles["Normal"], fontSize=8, textColor=colors.HexColor("#888888"), leading=11)
    ...
    header_data = [
        [
            ...
            Paragraph(f"INVOICE", ParagraphStyle("InvLabel", parent=styles["Heading1"], fontSize=28, alignment=2, textColor=colors.HexColor("#e63946"))),
        ],
        [
            Paragraph(company.get("address", "").replace("\n", "<br/>"), small_style),
            Paragraph(f"<b>Invoice #:</b> {inv_number}", ParagraphStyle("Right", parent=normal_style, alignment=2)),
        ],
        [
            Paragraph(f"{company.get('email', '')}  |  {company.get('phone', '')}", small_style),
            Paragraph(f"<b>Date:</b> {invoice.get('issued_date') or datetime.now().strftime('%B %d, %Y')}", ParagraphStyle("Right", parent=normal_style, alignment=2)),
        ],
        [
            Paragraph(f"GST #: {company.get('gst_number', 'N/A')}", small_style),
            Paragraph(f"<b>Due:</b> {invoice.get('due_date', 'N/A')}", ParagraphStyle("Right", parent=normal_style, alignment=2)),
        ],
    ]
    ...
    divider.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, -1), colors.HexColor("#e63946")), ("LINEBELOW", (0, 0), (-1, -1), 0, colors.white)]))
    ...
    elements.append(Paragraph("Payment is due within 30 days of invoice date. Please include the invoice number with your payment.", normal_style))
    elements.append(Spacer(1, 0.3 * inch))

    # Footer
    elements.append(Paragraph(f"Thank you for your business!", ParagraphStyle("Thanks", parent=styles["Normal"], fontSize=11, fontName="Helvetica-Oblique", textColor=colors.HexColor("#555555"), alignment=1)))
